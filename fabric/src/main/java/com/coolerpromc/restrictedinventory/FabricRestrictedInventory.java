...
public class FabricRestrictedInventory implements ModInitializer {
    ...
    public static final AttachmentType<Map<Integer, String>> RESTRICTED_SLOTS_ATTACHMENT =
            AttachmentRegistry.create(Constants.id("restricted_slots_attachment"), b ->
                    b.copyOnDeath()
                            .initializer(Map::of)
                            .syncWith(ByteBufCodecs.map(HashMap::new, ByteBufCodecs.INT, ByteBufCodecs.STRING_UTF8), AttachmentSyncPredicate.all())
    ...
    public void onInitialize() {
        ...
        PayloadTypeRegistry.playC2S().register(ServerBoundRestrictionUpdatePacket.TYPE, ServerBoundRestrictionUpdatePacket.STREAM_CODEC);
        PayloadTypeRegistry.playS2C().register(ClientBoundNotifyUpdatePacket.TYPE, ClientBoundNotifyUpdatePacket.STREAM_CODEC);
        PayloadTypeRegistry.playS2C().register(ClientBoundCommonConfigSyncPacket.TYPE, ClientBoundCommonConfigSyncPacket.STREAM_CODEC);
