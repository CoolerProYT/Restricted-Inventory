...
public final class RestrictionCodecs {
    public static final Codec<String> ITEM_OR_TAG_ID = ExtraCodecs.validate(Codec.STRING, RestrictionCodecs::validateItemOrTagId);
    public static final Codec<String> ITEM_ID = ExtraCodecs.validate(Codec.STRING, RestrictionCodecs::validateItemId);
    ...
    public static final Codec<String> SLOT_INDEX = ExtraCodecs.validate(Codec.STRING, RestrictionCodecs::validateSlotIndex);
    public static final MapCodec<Optional<CompoundTag>> NBT_FIELD = Codec.mapPair(
        CompoundTag.CODEC.optionalFieldOf("tag"),
        CompoundTag.CODEC.optionalFieldOf("nbt")
    ).xmap(
        pair -> pair.getFirst().isPresent() ? pair.getFirst() : pair.getSecond(),
        tag -> Pair.of(tag, Optional.empty())
    );
    ...
    public static <F, S> Codec<Either<F, S>> alternative(Codec<F> first, Codec<S> second) {
        return new Codec<>() {
            ...
            public <T> DataResult<Pair<Either<F, S>, T>> decode(DynamicOps<T> ops, T input) {
                ...
                String firstError = firstRead.error().map(DataResult.PartialResult::message).orElse("unknown");
                String secondError = secondRead.error().map(DataResult.PartialResult::message).orElse("unknown");
    ...
    private static DataResult<String> validateItemId(String value) {
        ...
        return ResourceLocation.isValidResourceLocation(value) ? DataResult.success(value) : DataResult.error(() -> "Not a valid item id: " + value);
    }
