...
public record GroupEntry(ResourceLocation group, Optional<DisplayEntry> display) implements Restriction {
    ...
    public boolean matches(ItemStack stack) {
        return RestrictionGroups.get(group).map(value -> value.matches(stack)).orElse(false);
    }
    ...
    public List<ItemStack> displayStacks() {
        ...
        return RestrictionGroups.get(group).map(RestrictionGroup::displayStacks).orElseGet(List::of);
    }
}
