...
- Group members can be item ids, `#tags`, or item + NBT entries; a group cannot contain another group
...
- Config entries accept `nbt` as an alias for `tag`, both are read and `tag` is what gets written back out
...
Item tags are still the better tool for grouping normal, distinct item ids. Restriction groups are for the combinations a tag cannot express, above all several NBT variants of the same item id.
